...
#' @param \dots Other arguments to pass to \code{\link{coxDKplsDR}}.
...
#' @seealso See Also \code{\link{coxDKplsDR}}
...
cv.coxDKplsDR =
            ...
            details=FALSE, namedataset="data", save=FALSE, verbose=TRUE,...)
  {
    ...
    for (i in seq(nfold)) {
      ...
      if(!file.exists(paste("cv.coxDKplsDR_",namedataset,"_folds_",i,".RData",sep=""))){
        assign(paste("cv.coxDKplsDR_",namedataset,"_folds_",i,sep=""),coxDKplsDR(Xplan=trdata$x, time=trdata$time, event=trdata$status, ncomp=nt, allres=TRUE, scaleX=TRUE, verbose=verbose, ...))
        if(save){save(list=c(paste("cv.coxDKplsDR_",namedataset,"_folds_",i,sep="")),file=paste("cv.coxDKplsDR_",namedataset,"_folds_",i,".RData",sep=""))}
      } else {
        load(paste("cv.coxDKplsDR_",namedataset,"_folds_",i,".RData",sep=""))
      }
      
      coeffit <- get(paste("cv.coxDKplsDR_",namedataset,"_folds_",i,sep=""))$CoeffCFull
      
      newdataAUC <- kernelMatrix(get(paste("cv.coxDKplsDR_",namedataset,"_folds_",i,sep=""))$kernDKplsDR_mod,
                                           scale(tsdata$x,
                                                 get(paste("cv.coxDKplsDR_",namedataset,"_folds_",i,sep=""))$XplanCent,
                                                 get(paste("cv.coxDKplsDR_",namedataset,"_folds_",i,sep=""))$XplanScal),
                                           scale(trdata$x,
                                                 get(paste("cv.coxDKplsDR_",namedataset,"_folds_",i,sep=""))$XplanCent,
                                                 get(paste("cv.coxDKplsDR_",namedataset,"_folds_",i,sep=""))$XplanScal))
      alldataAUC <- kernelMatrix(get(paste("cv.coxDKplsDR_",namedataset,"_folds_",i,sep=""))$kernDKplsDR_mod,
                                           scale(x,
                                                 get(paste("cv.coxDKplsDR_",namedataset,"_folds_",i,sep=""))$XplanCent,
                                                 get(paste("cv.coxDKplsDR_",namedataset,"_folds_",i,sep=""))$XplanScal),
                                           scale(trdata$x,
      ...
      for(jj in 1:nt){
        newxdata= (predict.pls.cox(get(paste("cv.coxDKplsDR_",namedataset,"_folds_",i,sep=""))$DKplsDR_mod,
                                   newdata=as.data.frame(newdataAUC),
                                   scale.X=FALSE,scale.Y=FALSE)$variates)[,1:jj,drop=FALSE]
        oldxdata=(as.matrix(get(paste("cv.coxDKplsDR_",namedataset,"_folds_",i,sep=""))$tt_DKplsDR))[,1:jj,drop=FALSE]
        allxdata=(predict.pls.cox(get(paste("cv.coxDKplsDR_",namedataset,"_folds_",i,sep=""))$DKplsDR_mod,
                                  newdata=as.data.frame(alldataAUC),
                                  scale.X=FALSE,scale.Y=FALSE)$variates)[,1:jj,drop=FALSE]
...
#      }
      ...
      rm(list=c(paste("cv.coxDKplsDR_",namedataset,"_folds_",i,sep="")))
    }
    if(allCVcrit){
    for(ind in 1:number_ind) {
    ...
    if(details){object <- c(object,list(All_indics=AUCs))}
      ...
      object <- list(nt=nt,cv.error10=cv.error10,cv.se10=cv.se10,folds=folds,lambda.min10=lamin10[[1]],lambda.1se10=lamin10[[2]])
