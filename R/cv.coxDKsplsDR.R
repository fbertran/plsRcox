...
#' 
#' This function cross-validates \link{coxDKsplsDR} models.\cr
#' 
...
#' variables passed to \code{\link{coxDKsplsDR}}'s \code{Xplan} argument,
#' \item\code{time} passed to \code{\link{coxDKsplsDR}}'s \code{time} argument,
#' \item\code{status} \code{\link{coxDKsplsDR}}'s \code{status} argument.  }
...
#' @param \dots Other arguments to pass to \code{\link{coxDKsplsDR}}.
...
#' @seealso See Also \code{\link{coxDKsplsDR}}
...
cv.coxDKsplsDR =
            ...
            details=FALSE, namedataset="data", save=FALSE, verbose=TRUE,...)
  {
    ...
    #on.exit(try(unloadNamespace("kernlab"),silent=TRUE))
    ...
    for (i in seq(nfold)) {
      ...
      if(!file.exists(paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,".RData",sep=""))){
        assign(paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,sep=""),coxDKsplsDR(Xplan=trdata$x, time=trdata$time, event=trdata$status, ncomp=nt, eta=eta, allres=TRUE, scaleX=TRUE, scaleY=FALSE, verbose=verbose, ...))
        if(save){save(list=c(paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,sep="")),file=paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,".RData",sep=""))}
      } else {
        load(paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,".RData",sep=""))
      }
      
      coeffit <- get(paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,sep=""))$CoeffCFull
      
      newdataAUC <- kernelMatrix(get(paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,sep=""))$kernDKsplsDR_mod,
                                           scale(tsdata$x,
                                                 get(paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,sep=""))$XplanCent,
                                                 get(paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,sep=""))$XplanScal),
                                           scale(trdata$x,
                                                 get(paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,sep=""))$XplanCent,
                                                 get(paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,sep=""))$XplanScal))
      alldataAUC <- kernelMatrix(get(paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,sep=""))$kernDKsplsDR_mod,
                                           scale(x,
                                                 get(paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,sep=""))$XplanCent,
                                                 get(paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,sep=""))$XplanScal),
                                           scale(trdata$x,
      ...
      nzb <- cumsum(c(0,sapply(get(paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,sep=""))$DKsplsDR_mod$new2As,length)))
      
      for(jj in 1:nt){
        Avalues <- get(paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,sep=""))$DKsplsDR_mod$A        
        
        newxdata=(predict.pls.cox(get(paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,sep=""))$DKsplsDR_modplsr,
                                  newdata=as.data.frame(newdataAUC)[,Avalues,drop=FALSE],
                                  scale.X=FALSE,scale.Y=FALSE)$variates)[,1:jj,drop=FALSE]
        oldxdata=(as.matrix(get(paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,sep=""))$tt_DKsplsDR))[,1:jj,drop=FALSE]
        allxdata=(predict.pls.cox(get(paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,sep=""))$DKsplsDR_modplsr,
                                  newdata=as.data.frame(alldataAUC)[,Avalues,drop=FALSE],
                                  scale.X=FALSE,scale.Y=FALSE)$variates)[,1:jj,drop=FALSE]
...
#      }
      ...
      rm(list=c(paste("cv.coxDKsplsDR_",namedataset,"_folds_",i,"_eta_",eta,sep="")))
    }
    if(allCVcrit){
    for(ind in 1:number_ind) {
