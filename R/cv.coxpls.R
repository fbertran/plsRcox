...
#' 
#' This function cross-validates \link{coxpls} models.\cr
#' 
...
#' variables passed to \code{\link{coxpls}}'s \code{Xplan} argument,
#' \item\code{time} passed to \code{\link{coxpls}}'s \code{time} argument,
#' \item\code{status} \code{\link{coxpls}}'s \code{status} argument.  }
...
#' @param \dots Other arguments to pass to \code{\link{coxpls}}.
...
#' @seealso See Also \code{\link{coxpls}}
...
cv.coxpls =
            ...
            details=FALSE, namedataset="data", save=FALSE, verbose=TRUE,...)
  {
    ...
    for (i in seq(nfold)) {
      ...
      if(!file.exists(paste("cv.coxpls_",namedataset,"_folds_",i,".RData",sep=""))){
        assign(paste("cv.coxpls_",namedataset,"_folds_",i,sep=""),coxpls(Xplan=trdata$x, time=trdata$time, event=trdata$status, ncomp=nt, allres=TRUE, verbose=verbose, ...))
        if(save){save(list=c(paste("cv.coxpls_",namedataset,"_folds_",i,sep="")),file=paste("cv.coxpls_",namedataset,"_folds_",i,".RData",sep=""))}
      } else {
        load(paste("cv.coxpls_",namedataset,"_folds_",i,".RData",sep=""))
      }
      
      coeffit <- get(paste("cv.coxpls_",namedataset,"_folds_",i,sep=""))$CoeffCFull
      
      
      for(jj in 1:nt){
        
        newxdata=(predict.pls.cox(get(paste("cv.coxpls_",namedataset,"_folds_",i,sep=""))$pls_mod,
                                  newdata=scale(tsdata$x,
                                                get(paste("cv.coxpls_",namedataset,"_folds_",i,sep=""))$XplanCent,
                                                get(paste("cv.coxpls_",namedataset,"_folds_",i,sep=""))$XplanScal),
                                  scale.X=FALSE,scale.Y=FALSE)$variates)[,1:jj,drop=FALSE]
        oldxdata=(as.matrix(get(paste("cv.coxpls_",namedataset,"_folds_",i,sep=""))$tt_pls))[,1:jj,drop=FALSE]
        
        allxdata=(predict.pls.cox(get(paste("cv.coxpls_",namedataset,"_folds_",i,sep=""))$pls_mod,
                                  newdata=scale(x,
                                                get(paste("cv.coxpls_",namedataset,"_folds_",i,sep=""))$XplanCent,
                                                get(paste("cv.coxpls_",namedataset,"_folds_",i,sep=""))$XplanScal),
                                  scale.X=FALSE,scale.Y=FALSE)$variates)[,1:jj,drop=FALSE]
...
#      }
      ...
      rm(list=c(paste("cv.coxpls_",namedataset,"_folds_",i,sep="")))
    }
    if(allCVcrit){
    for(ind in 1:number_ind) {
