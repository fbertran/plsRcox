...
cv.plsRcox =
            ...
            details=FALSE, namedataset="data", save=FALSE, verbose=TRUE,...)
  {
    ...
    for (i in seq(nfold)) {
      ...
      if(!file.exists(paste("cv.plsRcox_",namedataset,"_folds_",i,".RData",sep=""))){
        assign(paste("cv.plsRcox_",namedataset,"_folds_",i,sep=""),plsRcox(Xplan=trdata$x, time=trdata$time, event=trdata$status, method = method, nt=nt, allres=TRUE, verbose=verbose, ...))
        if(save){save(list=c(paste("cv.plsRcox_",namedataset,"_folds_",i,sep="")),file=paste("cv.plsRcox_",namedataset,"_folds_",i,".RData",sep=""))}
      } else {
        load(paste("cv.plsRcox_",namedataset,"_folds_",i,".RData",sep=""))
      }
      
      coeffit=as.matrix(get(paste("cv.plsRcox_",namedataset,"_folds_",i,sep=""))$CoeffCFull)
      newxdata=as.matrix(predict(get(paste("cv.plsRcox_",namedataset,"_folds_",i,sep="")),newdata=tsdata$x, comps=min(nt,get(paste("cv.plsRcox_",namedataset,"_folds_",i,sep=""))$computed_nt), type="scores"))
      oldxdata=as.matrix(predict(get(paste("cv.plsRcox_",namedataset,"_folds_",i,sep="")), comps=min(nt,get(paste("cv.plsRcox_",namedataset,"_folds_",i,sep=""))$computed_nt), type="scores"))
      allxdata=as.matrix(predict(get(paste("cv.plsRcox_",namedataset,"_folds_",i,sep="")),newdata=x, comps=min(nt,get(paste("cv.plsRcox_",namedataset,"_folds_",i,sep=""))$computed_nt), type="scores"))
      ...
      for(jj in 1:min(nt,get(paste("cv.plsRcox_",namedataset,"_folds_",i,sep=""))$computed_nt)){
        ...
        predict.trainvectjj <- as.matrix(predict(get(paste("cv.plsRcox_",namedataset,"_folds_",i,sep="")),newdata=trdata$x, comps=jj, type="lp"))
        predictvectjj <- as.matrix(predict(get(paste("cv.plsRcox_",namedataset,"_folds_",i,sep="")),newdata=tsdata$x, comps=jj, type="lp"))
      ...
      rm(list=c(paste("cv.plsRcox_",namedataset,"_folds_",i,sep="")))
    }
    if(allCVcrit){
    for(ind in 1:number_ind) {
